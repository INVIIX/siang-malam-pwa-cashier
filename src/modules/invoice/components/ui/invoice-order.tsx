...
import { displayOrderItems } from "../../helpers/customer-display"
...
export function InvoiceOrderSearch({
    className,
    value,
    onChangeValue,
}: { value: string; onChangeValue: (value: string) => void } & React.ComponentProps<"div">) {
    return (
        <>
            <div className={cn("w-full h-auto relative", className)}>
                ...
                <Input
                    type="search"
                    className="ps-11 bg-white w-full"
...
export function InvoiceOrder({
    ...
    handleAction,
    ...props
}: { handleAction: (invoice: IInvoice) => void } & React.ComponentProps<"div">) {
...
export function InvoiceOrderTableInfo({
    ...
    useEffect(() => {
        mDisplayOrderItems().catch((error) => {
            console.error("Failed to display order items:", error)
        })
    ...
    return (
        <>
            <Table className={cn(["w-full border-separate border-spacing-0", className])} {...props}>
                ...
                <TableBody>
                    {invoice.transaction.details.map((item, key) => (
                        <TableRow key={key}>
