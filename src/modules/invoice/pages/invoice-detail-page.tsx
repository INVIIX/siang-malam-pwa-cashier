...
import { encodeReceipt, previewReceipt } from "../helpers/receipt-printer-text";
...
export default function dataDetailPage() {
    ...
    useEffect(() => {
        if (data) {
            setPrinterCommands(encodeReceipt(data, user?.department?.name ?? 'RM. Siang Malam', user?.branch?.name ?? '-'))
        }
    }, [data])
                        ...
                        <Card className="w-full">
                            ...
                            <CardContent className="grid gap-4">
                                <div className="max-w-full mx-auto overflow-x-auto">
                                    <pre>{previewReceipt(data, user?.department?.name ?? 'RM. Siang Malam', user?.branch?.name ?? '-')}</pre>
                                </div>
